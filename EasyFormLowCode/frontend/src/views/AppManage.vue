  ...
  <section class="manage-page">
    <header class="page-header">
      <div>
        <span>Projects</span>
        <h1>项目与页面</h1>
        <p>每个页面从可运行的后台 CRUD 模板开始，再进入设计器配置。</p>
      </div>
    ...
    <div class="manage-grid">
      <section class="panel project-panel">
        ...
        >
          ...
          <small>{{ project.page_count }} 个页面</small>
        ...
        <el-table v-else :data="pages" size="default">
          ...
          <el-table-column label="状态" width="110">
            <template #default="{ row }">
              <el-tag :type="row.has_published ? 'success' : 'warning'" effect="plain">
                {{ row.has_published ? '已发布' : '草稿' }}
              </el-tag>
            </template>
          </el-table-column>
          <el-table-column label="操作" width="220" fixed="right">
            <template #default="{ row }">
              <el-button link type="primary" @click="designPage(row)">设计</el-button>
              <el-button link @click="openPageDialog(row)">重命名</el-button>
    ...
    <el-dialog v-model="projectDialogVisible" :title="editingProject ? '重命名项目' : '新建项目'" width="420px">
      <el-form label-position="top" @submit.prevent="submitProject">
    ...
    <el-dialog v-model="pageDialogVisible" :title="editingPage ? '重命名页面' : '新建后台 CRUD 页面'" width="460px">
      <el-form label-position="top" @submit.prevent="submitPage">
        <el-form-item label="页面名称" required>
          <el-input v-model="pageForm.name" maxlength="120" />
        </el-form-item>
        <el-form-item v-if="!editingPage" label="页面标识" required>
          <el-input v-model="pageForm.pageId" placeholder="例如 user_manage" />
          <small>仅支持英文字母开头、数字、下划线和连字符。</small>
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="pageDialogVisible = false">取消</el-button>
        <el-button type="primary" :loading="submitting" @click="submitPage">创建并配置</el-button>
      </template>
    </el-dialog>
  </section>
</template>
